...
pub fn auto_generate_loader() {
    println!("cargo:rerun-if-env-changed=LENTO_JS_DIR");
    if let Ok(js_dir) = env::var("LENTO_JS_DIR") {
        generate_static_loader(js_dir.as_str(), "target/lento_js_loader.in");
    } else {
        let js_url = env::var("LENTO_JS_URL").unwrap_or("http://localhost:7800".to_string());
        generate_dev_loader(js_url.as_str(), "target/lento_js_loader.in");
    }
}

pub fn generate_dev_loader(url: &str, output_dir: &str) {
    let mut code = format!("Box::new(lento::loader::DevModuleLoader::new(Some(\"{}\")))", url);
...
pub fn generate_static_loader(js_dir: &str, output_dir: &str) {
    ...
    code.push_str("{\n");
    code.push_str("let mut loader = lento::loader::StaticModuleLoader::new();\n");
    for f in files {
