...
pub fn build_border_paths(
    ...
    for (top, left, left_radius, right, right_radius, width, height, rotate, tx, ty) in borders {
        if let Some(mut path) = draw_top_border(
            &top,
            &left,
            left_radius,
            &right,
...
fn draw_top_border(
    top_border: &BorderProps,
    left_border: &BorderProps,
    left_radius: f32,
    right_border: &BorderProps,
    right_radius: f32,
    width: f32,
    height: f32,
) -> Option<Path> {
    ...
    clip_ps.push(build_radius_clip(
        border_width,
        left_border_width,
        left_radius,
        width,
...
pub fn build_rect_with_radius(radius: [f32; 4], width: f32, height: f32) -> Path {
    ...
    p.arc_to(
        Rect::new(0.0, 0.0, radius[0] * 2.0, radius[0] * 2.0),
        180.0,
        90.0,
        false,
    );
    ...
    p.arc_to(
        Rect::new(width - radius[1] * 2.0, 0.0, width, radius[1] * 2.0),
        270.0,
        90.0,
        false,
    );
    ...
    p.arc_to(
        Rect::new(0.0, height - radius[3] * 2.0, radius[3] * 2.0, height),
        90.0,
        90.0,
        false,
    );
...
fn build_radius_path(left_radius: f32, width: f32, height: f32) -> Path {
    ...
    p.arc_to(
        Rect::new(0.0, 0.0, left_radius, left_radius),
        180.0,
        90.0,
        false,
    );
...
fn build_radius_clip(
    ...
    if left_radius > left_border_width && left_radius > border_width {
        ...
        let rect = Rect::new(
            left_border_width,
            border_width,
            left_border_width + inner_arc_width,
            border_width + inner_arc_height,
        );
        ...
        p.add_rect(
            Rect::new(left_border_width, border_width, width, height),
            None,
        );
    }
    p
