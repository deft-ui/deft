...
fn ellipse_arc_angle(pb: &mut PathBuilder, cx: f32, cy: f32, a: f32, b: f32, start_angle: f32, end_angle: f32, clockwise: bool) {
    ellipse_arc(pb, cx, cy, a, b, start_angle * PI / 180.0, end_angle * PI / 180.0, clockwise);
}

fn ellipse_arc(pb: &mut PathBuilder, cx: f32, cy: f32, a: f32, b: f32, start_angle: f32, end_angle: f32, clockwise: bool) {
...
fn build_corner(border_width: [f32; 2], radius: f32, transform: Option<Transform>) -> Option<Path> {
    ...
    ellipse_arc(&mut pb, left_radius, left_radius, left_radius, left_radius, start_angle, end_angle, false);
    ...
    if radius > border_width[0] && radius > border_width[1] {
        ...
        ellipse_arc(&mut pb, left_radius, left_radius, left_radius - border_width[0], left_radius - border_width[1], start_deg, end_deg, true);
    } else {
...
pub fn build_rect_with_radius(radius: [f32; 4], width: f32, height: f32) -> Option<Path> {
    ...
    if radius[0] == 0.0 && radius[0] == radius[1] && radius[1] == radius[2] && radius[2] == radius[3] {
    ...
    ellipse_arc_angle(&mut p, radius[0], radius[0], radius[0], radius[0], 180.0, 270.0, false);

    p.line_to(width - radius[1], 0.0);
    ellipse_arc_angle(&mut p, width - radius[1], radius[1], radius[1], radius[1], 270.0, 360.0, false);

    p.line_to(width, height - radius[2]);
    ellipse_arc_angle(&mut p, width - radius[2], height - radius[2], radius[2], radius[2], 0.0, 90.0, false);

    p.line_to(radius[3], height);
    ellipse_arc_angle(&mut p, radius[3], height - radius[3], radius[3], radius[3], 90.0, 180.0, false);

    p.close();
    p.finish()
}

pub fn build_border_paths(border_width: [f32; 4], radius: [f32; 4], width: f32, height: f32) -> (Option<Path>, Option<Path>, Option<Path>, Option<Path>) {
    ...
    let top_line = build_line(top_line_space1, 0.0, width - top_line_space1 - top_line_space2, bt);
    ...
    let right_transform2 = Transform::from_scale(-1.0, 1.0).post_translate(height, 0.0).post_concat(right_transform1);
    ...
    let right_line = build_line(width - br, right_line_space1, br, height - right_line_space1 - right_line_space2);
    ...
    let bottom_transform2 = Transform::from_scale(-1.0, 1.0).post_translate(width, 0.0).post_concat(bottom_transform1);
    ...
    let bottom_line = build_line(bottom_line_space2, height - bb, width - bottom_line_space1 - bottom_line_space2, bb);
    ...
    let left_transform2 = Transform::from_scale(-1.0, 1.0).post_translate(height, 0.0).post_concat(left_transform1);
    ...
    let left_line = build_line(0.0, left_line_space2, bl, height - left_line_space1 - left_line_space2);
