...
impl FontManager {
    ...
    fn get_by_family_name(&self, name: &str, expected_font_style: &skia_safe::FontStyle) -> Option<Font> {
        ...
        me.cache.entry(cache_key).or_insert_with(move || {
            ...
            Self::select_best_font_by_weight(result, weight).map(|f| {
                f.synthesize(Weight(weight), expected_style)
            })
        }).clone()
    }
